...
export function buildFactsPrompt(args: GenerateSbarArgs): string {
  ...
  if (args.trendFindings.length > 0) {
    lines.push(
      "",
...
function formatSymptoms(symptoms: Symptoms): string {
  ...
  for (const key of Object.keys(SYMPTOM_LABELS) as Array<keyof typeof SYMPTOM_LABELS>) {
    const value = symptoms[key];
    if (value === true) {
      reported.push(SYMPTOM_LABELS[key]);
