import { Symptoms, VitalsReading, EcgDetermination, EcgReading } from "@/lib/clinical/types";
...
export interface Vignette {
  ...
  /** The engine takes ECG as a separate reading, not as a field on vitals. */
  ecg?: EcgReading;
}
...
const ECG_OPTIONS: EcgDetermination[] = [
  "normal_sinus_rhythm",
  "tachycardia",
  "atrial_fibrillation",
  "bradycardia",
...
export function generateVignettes(n: number, seed = 20260726): Vignette[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // ECG is present on most but not all check-ins — a patch that was not worn
    // is a real state the engine has to reason about.
    const ecg: EcgReading | undefined =
      r() < 0.75
        ? {
            recordedAt: `synthetic-${i}`,
            determination: pick(r, ECG_OPTIONS),
            source: "kardia_6l",
          }
        : undefined;

    out.push({ id: `vig-${String(i).padStart(5, "0")}`, dayPostOp, symptoms, vitals, ecg });
  }
