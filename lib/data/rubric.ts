import { Severity, Symptoms, VitalsReading, EcgReading } from "@/lib/clinical/types";
...
interface RubricContext {
  ...
  s: Symptoms;
  v: VitalsReading;
  /** Taken as-is from the device; the rubric never re-derives a rhythm. */
  ecg?: EcgReading;
...
const RULES: RubricRule[] = [
  // ---- Immediately life-threatening ----
  {
    ...
    severity: "red",
    weight: 100,
    applies: ({ s, v, measured, ecg }) =>
      Boolean(s.breathless || s.chestPain) &&
      (!measured ||
        (v.hr !== undefined && v.hr > 110) ||
        Boolean(ecg?.determination === "tachycardia")),
    because:
  ...
  },
  {
    ...
    severity: "amber",
    weight: 55,
    applies: ({ ecg }) => ecg?.determination === "atrial_fibrillation",
...
export function rubricSeverity(input: {
  ...
  const clean: VitalsReading = trustworthy
    ...
    : { timestamp: v.timestamp, quality: v.quality, source: v.source };
